...
export class AudioStreamer {
  ...
  constructor(public context: AudioContext) {
    // Use the AudioContext's actual sample rate, not a hardcoded value
  ...
  /**
   * Decode audio from a Blob (e.g., Opus/WebM) and add to playback queue
   */
  async addEncodedAudio(blob: Blob): Promise<void> {
    try {
  ...
  /**
   * Add PCM16 audio data (as base64 string or Uint8Array) to the playback queue
   */
  addPCM16(chunk: Uint8Array | string) {
    ...
    let processingBuffer = this._processPCM16Chunk(uint8Array);
